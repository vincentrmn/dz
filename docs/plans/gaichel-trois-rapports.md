...
| 3 | Non-régression : fiche 1 (wbs inchangé) régénérée sur W donne le même « Traxxeo : N ligne(s) » avant et après le changement du nœud | Deux lignes `GET /api/runs` fiche 1, période W, même N, statut Succès | écart accepté (preuve « même N » non obtenue ; obtenu : RT/BLL identiques et N ≥ avant) : avant = run 314 « Succès · Traxxeo : 0 ligne(s) · RT 0/0 · BLL 1 message, 0 photo » ; après = run 315 « Succès · Traxxeo : **31 ligne(s)** · RT 0/0 · BLL 1 message, 0 photo » (21/09, `.verif/runs-314-315.txt`). N n'est pas égal parce que Traxxeo renvoie **déjà** `22.06A`/`22.06B` sans espace (Fares a fait le changement) : visible dans l'exécution n8n 9692, sortie de `Mapper activité Traxxeo`, libellés de repli « Heure travail — 22.06A » / « … 22.06B ». Le chapitre 1 de Gaichel était vide en silence depuis fin août (runs 291, 303, 310 à 0 ligne) ; la normalisation le rétablit sans toucher aux autres chapitres |
| 4 | Trois fiches distinctes et complètes | `GET /api/chantiers` : noms contenant `22.06A`, `22.06B`, `22.06C`, six IDs de conversation non vides et tous différents, wbs `22.06A`/`22.06B`/`22.06C`, `actif=true`, `mail_actif=true`, emails `dzconstruct@dzconstruct.lu` | réussi (22/09) : après un premier message posté par DZ dans les quatre conversations A et C (Teams ne crée une conversation de groupe côté serveur qu'au premier message : avant, invisibles pour Graph), scan relancé (exécution 10006, Succès) → fiches 19 et 21 complétées, six IDs de conversation non vides et tous distincts sur A/B/C, aucun doublon Gaichel, une seule autre écriture = nouvelle fiche 22 `26.05-Logements-Leudelange` inactive (`.verif/diff-chantiers-scan-AC.txt`). Historique du 21/09 : partiel : `GET /api/chantiers` → fiches 19 `22.06A Gaichel-Maisons`, 20 `22.06B Gaichel-Maisons`, 21 `22.06C Gaichel-Maisons`, wbs `22.06A`/`22.06B`/`22.06C`, `actif=true`, `mail_actif=true`, emails `dzconstruct@dzconstruct.lu` (`.verif/chantiers-apres-creation.json`). **Il manque 4 IDs sur 6** : seule B a ses deux conversations. L'exécution 9685 du scan liste 232 conversations du compte assembleur ; les seules Gaichel sont `22.06B-BL&L-Gaichel-Maisons` (`19:f1f06e61…`) et `22.06B-RT-Gaichel-Maisons` (`19:9e38aca9…`), c'est-à-dire les **anciennes conversations de la fiche 1, renommées en B**. Aucune `22.06A-…` ni `22.06C-…` visible : le compte assembleur n'en est pas membre (ou elles n'existent pas). Vincent a demandé à Francis d'ajouter le compte assembleur aux conversations A et C ; les IDs se saisiront dans le Dashboard (« Configuration des sources ») dès qu'ils seront visibles |
| 5 | Ancienne fiche conservée et inactive | Même appel : fiche 1 `actif=false`, `supprime=false`, wbs et conversations identiques au relevé du début du lot 1 ; `GET /api/reports` liste toujours `22.06-Gaichel-Maisons__*` | réussi : diff fiche 1 entre `.verif/chantiers-debut-lot1.json` et `.verif/chantiers-apres-creation.json` = `{'actif': (True, False)}` seulement, `supprime=false` ; aucune autre fiche modifiée hormis la 19, restaurée et remplie comme fiche A (prévu au périmètre) ; `GET /api/reports` (jeton) → 66 fichiers `22.06-Gaichel-Maisons__*` toujours listés, jusqu'à `2026-09-14_2026-09-20` |
...
| 8 | La découverte crée une fiche distincte pour un code à lettre, quelle que soit la graphie, et ne touche pas les fiches existantes | Sortie d'exécution du scan relancé (MCP n8n) : aucun doublon, diff vide de `GET /api/chantiers` avant/après ; `node -e` sur la dérivation de clé : `22.06A-BL&L-X`, `22.06 A-RT-X`, `CH:22-06 A` donnent `22.06A`, `22.06-BL&L-X` donne `22.06`, sort 0 | réussi (21/09) : nœud `Rapprocher découvertes` de `49okCW9O85lYsP3r` réécrit par `update_workflow` (lettre collée en majuscule, `\s*([A-Z])?` au lieu de `(\s+[A-Z])?`, clé d'une fiche = code du nom puis du WBS, nom créé avec la lettre collée) puis `publish_workflow` → `{success:true, activeVersionId:6844d6fd…}`, relecture `get_workflow_details` : brouillon = version active, jsCode identique au fichier testé (`.verif/rapprocher-decouvertes.js`, sha256 `9ab9d055…`). Dérivation de clé : `node .verif/test-decouverte.js` → `22.06A-BL&L-X`, `22.06 A-RT-X`, `CH:22-06 A`, `22.06 a-RT-X` → `22.06A` ; `22.06-BL&L-X` et `22.06 Gaichel-RT-X` → `22.06` ; l'ancien code donnait `22.06` pour `22.06A-BL&L-X` (rabattement). 18 cas passants, exit 0, dont : fiches réelles de prod + conversations synthétiques `22.06A-…` / `22.06 C-…` → complètent les fiches 19 et 21 sans rien créer ; `22.06 D` → nouvelle fiche `22.06D` inactive ; code d'une fiche supprimée → rien. Scan réel relancé (exécution 9769, Succès, Graph actif, page 1 = 46 conversations dont `22.06B-BL&L/RT-Gaichel-Maisons`, nœud source de `Rapprocher découvertes` = `Lister conversations Teams` ; les deux nœuds référencés par le code, `Charger chantiers existants` et `Lister conversations Teams`, existent sous ce nom exact dans `get_workflow_details`) : `Rapprocher découvertes` sort 0 ligne, `GET /api/chantiers` avant/après : 18 fiches, 0 différence (`.verif/diff-chantiers-scan-lot2.txt`) |
| 9 | Le guide documente la convention | `grep -c "22.06A-BL&amp;L-Gaichel-Maisons" public/guide.html` ≥ 1 et `grep -c "22.06 A;22.06 B" public/guide.html` = 0 ; captures `guide-teams-1440.png` et `guide-teams-390.png` (servi en local, pas de scroll horizontal) ; après déploiement `curl …/guide` avec le jeton contient `22.06A` | réussi en local (21/09) : `grep -c "22.06A-BL&amp;L-Gaichel-Maisons" public/guide.html` = 1, `grep -c "22.06 A;22.06 B" public/guide.html` = 0 ; sections « Bien nommer les conversations Teams » (exemples `22.06A-BL&L-…`/`22.06A-RT-…`, nouveau paragraphe « chantier divisé en parties » : lettre collée dans Teams et Traxxeo, tolérance à l'espace, `22.06` distinct de `22.06A`) et « Activer un nouveau chantier » (exemple `22.06A`) ; captures `.verif/guide-teams-1440.png` (1440 px, `scrollWidth` = 1440, sans défilement) et `.verif/guide-teams-390.png` servies par `.verif/stub-server.js`. À 390 px la page déborde à 833 px **exactement comme sur `origin/main`** (`.verif/mesure-390.js` : `nav` du header 528 px et `table.tableau-contacts` 659 px, identiques avant/après) : préexistant, hors périmètre, la section modifiée tient dans 390 px. Volet « après déploiement » (21/09, PR #5 fusionnée, déploiement `39b2c671` SUCCESS) : `/guide` sans session → 302 `/login?suite=%2Fguide` ; le contenu `22.06A` en ligne n'a pas pu être relu (jeton non lisible par le MCP Railway, valeurs masquées), à confirmer par Vincent en session connectée |
| 10 | BIBLE et CLAUDE.md alignés | `grep -n "ne pas changer" CLAUDE.md` ne cite plus Gaichel ; `grep -c "22.06A" docs/BIBLE.md` ≥ 1 ; `grep -c "22.06 A + 22.06 B" CLAUDE.md` = 0 | réussi (21/09) : `grep -n "ne pas changer" CLAUDE.md` → aucune ligne (le CLAUDE.md réécrit à l'installation du workflow korr n'avait déjà plus ni la ligne pilote ni l'acquis `22.06 A + 22.06 B`) ; `grep -c "22.06A" docs/BIBLE.md` = 5 (§4 étape 6 : comparaison normalisée dans `Mapper activité Traxxeo` ; §5 : découverte avec lettre collée, rattachement par le nom puis le WBS ; §9 : « Échec Traxxeo silencieux » enrichi du cas Gaichel ; §12 : WBS `22.06A`) ; `grep -c "22.06 A + 22.06 B" CLAUDE.md` = 0 ; CLAUDE.md §8 mis à jour (trois fiches Gaichel, lettre collée, reste à faire), 123 lignes. Le piège « comparaison WBS normalisée » est dans `.claude/rules/n8n.md` depuis le lot 1, pas dans CLAUDE.md (qui ne grandit pas) |
| 11 | Cockpit sain | `node --check server.js` sort 0 ; `curl -s …/api/health` répond `"cockpit":"ok"` après déploiement | lot 2 (21/09) : `npm run check` sort 0 après les modifications du guide, de la BIBLE et de CLAUDE.md (`.verif/check.log`) ; après fusion de la PR #5 (`17def40`) et déploiement Railway `39b2c671` SUCCESS à 18:30 UTC, `curl -s …/api/health` → 200 `{"cockpit":"ok","n8n":"ok","stockage":"ok","rapports":477}`, routes gardées en 302/401, icônes en 200 : réussi. Lot 1 : réussi (volet lot 1) : `node --check server.js` sort 0 (21/09) ; après fusion de `main` (workflow korr), `npm run check` (syntaxe, eslint, 5 tests) sort 0 (`.verif/check.log`) ; `/api/health` en prod répond `"cockpit":"ok"` avant tout déploiement ; après déploiement `9d843a26` du 21/09 (merge #3), `curl -s …/api/health` → 200 `"cockpit":"ok"` |
